...
class modReports extends DolibarrModules
{
    ...
    public function __construct($db)
    {
        ...
        */
        ...
        $a = 0;
        $r = 0;
        $menuArray = [['Receipts (Collection)', 'receipts'],
        ['Receipts Compact', 'receipts_mobile'],
        ['Balances', 'balances'],
        ['Cashflow Daily Totals', 'cashflow_daily_totals'],
        ['Credit Notes (Returns)', 'Credit_Notes_Returns'],
        ['Finished', 'Finished_Group'],
        ['Monthly Comparison', 'Monthly_Collection_Totals'],
        ['Overdue', 'Overdue'],
        ['Open Check Points', 'Open_Check_Points'],
        ['Open Invoices', 'OpenInvoices'],
        ['Percentage Collection', 'Percentage_Collection'],
        ['Receipt Book Check', 'receipt_book_check'],
        ['Sales', 'Sales_Invoices'],
        ['Shop Papers', 'shop_papers'],
        ['Stock Balance (Stock Count)', 'Stock_Balance'],
        ['Unclassified Invoices', 'Unclassified_Invoices']
        ];
        ...
        $this->menu[$r++] = [
            'fk_menu' => 'fk_mainmenu=reports,fk_leftmenu=myobject_list',	    // '' if this is a top menu. For left menu, use 'fk_mainmenu=xxx' or 'fk_mainmenu=xxx,fk_leftmenu=yyy' where xxx is mainmenucode and yyy is a leftmenucode
            'type' => 'left',			                // This is a Left menu entry
            'titre' => $menuArray[14][0],
            'mainmenu' => 'reports',
            'leftmenu' => 'reports',
            'url' => '/reports/selection.php?reportid='.$menuArray[14][1],
            'langs' => 'reports@reports',	        // Lang file to use (without .lang) by module. File must be in langs/code_CODE/ directory.
            'position' => 1000 + $r,
            'enabled' => '$conf->reports->enabled',  // Define condition to show or hide menu entry. Use '$conf->reports->enabled' if entry must be visible if module is enabled. Use '$leftmenu==\'system\'' to show if leftmenu system is selected.
            'perms' => '$user->rights->reports->myobject_list->delete',			                // Use 'perms'=>'$user->rights->reports->level1->level2' if you want your menu with a permission rules
        ...
        $this->menu[$r++] = [
            'fk_menu' => 'fk_mainmenu=reports,fk_leftmenu=myobject_list',	    // '' if this is a top menu. For left menu, use 'fk_mainmenu=xxx' or 'fk_mainmenu=xxx,fk_leftmenu=yyy' where xxx is mainmenucode and yyy is a leftmenucode
            'type' => 'left',			                // This is a Left menu entry
            'titre' => $menuArray[15][0],
            'mainmenu' => 'reports',
            'leftmenu' => 'reports',
            'url' => '/reports/selection.php?reportid='.$menuArray[15][1],
            'langs' => 'reports@reports',	        // Lang file to use (without .lang) by module. File must be in langs/code_CODE/ directory.
            'position' => 1000 + $r,
            'enabled' => '$conf->reports->enabled',  // Define condition to show or hide menu entry. Use '$conf->reports->enabled' if entry must be visible if module is enabled. Use '$leftmenu==\'system\'' to show if leftmenu system is selected.
            'perms' => '$user->rights->reports->myobject_list->delete',			                // Use 'perms'=>'$user->rights->reports->level1->level2' if you want your menu with a permission rules
