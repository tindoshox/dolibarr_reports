...
class DailyTotalsReportPDF extends TCPDF
{
	public $reportTitle = '';
	public $dates = [];
	public $states = [];
	...
	public function renderCrosstab()
	{
		...
		$this->Cell($rowLabelWidth, 7, 'Date', 1, 0, 'C', true);
		foreach ($this->states as $state) {
			$this->Cell($cellWidth, 7, substr($state, 0, 6), 1, 0, 'C', true);
		}
		...
		foreach ($this->dates as $date) {
			...
			$this->Cell($rowLabelWidth, 6, date('d-M', strtotime($date)), 1, 0, 'C', true);

			$rowTotal = 0;
			foreach ($this->states as $state) {
				$amount = $this->getAmount($date, $state);
				$this->Cell($cellWidth, 6, $amount > 0 ? number_format($amount) : '-', 1, 0, 'C');
				$rowTotal += $amount;
			}

			$this->SetFillColor(0, 95, 179);
			$this->SetTextColor(255);
			$this->Cell($totalsColumnWidth, 6, number_format($rowTotal), 1, 1, 'C', true);
			$this->SetTextColor(0);
		}
		...
		$this->SetFillColor(0, 95, 179);
		$this->SetTextColor(255);
		$this->Cell($rowLabelWidth, 7, 'Total', 1, 0, 'C', true);
		...
		foreach ($this->states as $state) {
			...
			$grandTotal += $colTotal;
			$this->Cell($cellWidth, 7, number_format($colTotal), 1, 0, 'C', true);
		}
		$this->Cell($totalsColumnWidth, 7, number_format($grandTotal), 1, 1, 'C', true);
