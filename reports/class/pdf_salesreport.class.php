...
class SalesReportPDF extends TCPDF
{
	...
	public function Header()
	{
		...
		$this->Cell(0, 8, "SALES FOR : {$this->startdate} TO {$this->enddate}", 0, 0, 'C');
		$this->Ln(1);
	}
	...
	public function renderReport($data)
	{
		...
		foreach ($data as $inv) {
			...
			$this->Cell(25, 6,date('d-m-Y', strtotime($inv['date'])), 0, 0);
			$this->Cell(20,6, substr($inv['del_note'],0,26), 0, 0);
			$this->Cell(30,6, $inv['invoice'], 0, 0);
			...
			$this->Cell(15,6, number_format($inv['amount'],0), 0, 0,);
			$this->Cell(25,6, substr($inv['product'],0,10), 0, 0, );
			$this->Cell(20,6, $inv['user'], 0, 0, 'R');
			$this->Ln(6);
		}
	}
